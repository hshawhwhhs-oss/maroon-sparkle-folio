...
import { MapPin, Upload, Link as LinkIcon, Navigation, CheckCircle, AlertCircle, X, Camera, FileText, Send } from 'lucide-react';
...
export default function AddReportPage() {
  ...
  const [formData, setFormData] = useState({
    title: '',
    corruptionType: DEFAULT_CORRUPTION_TYPES[0].name,
    description: '',
    locationName: '',
    latitude: parseFloat(searchParams.get('lat') || '23.8103'),
    longitude: parseFloat(searchParams.get('lng') || '90.4125'),
  ...
  // Initialize map picker with vanilla Leaflet
  useEffect(() => {
    if (showMapPicker && mapPickerRef.current && !mapPickerInstance.current) {
      ...
      const marker = L.marker([formData.latitude, formData.longitude], { draggable: true }).addTo(map);
      ...
      <form onSubmit={handleSubmit}>
        ...
        {step === 1 && (
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-1.5">করাপশন টাইপ</label>
              <div className="grid grid-cols-3 gap-2">
                {DEFAULT_CORRUPTION_TYPES.map(type => (
                  <button
                    key={type.id}
                    type="button"
                    onClick={() => setFormData({ ...formData, corruptionType: type.name })}
                    className={`flex flex-col items-center p-3 rounded-xl border-2 transition-all text-center ${
                      formData.corruptionType === type.name
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-1.5">শিরোনাম</label>
              <input
                type="text"
                placeholder="যেমন: পাসপোর্ট অফিসে ঘুষ চাওয়া হয়েছে"
                value={formData.title}
                onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-red-500 outline-none"
                required
              />
            </div>

            <div>
              <label className="block text-xs font-bold text-gray-500 mb-1.5">বিবরণ</label>
              <textarea
                rows={4}
                placeholder="কি ঘটেছিল বিস্তারিত লিখুন..."
                value={formData.description}
                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-red-500 outline-none resize-none"
                required
              />
            </div>

            <button type="button" onClick={() => setStep(2)} className="w-full bg-red-600 text-white py-3 rounded-xl font-bold text-sm active:scale-[0.98] transition-all">
              পরবর্তী →
            </button>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-1.5">অবস্থানের নাম</label>
              <input
                type="text"
                placeholder="যেমন: আগারগাঁও পাসপোর্ট অফিস, ঢাকা"
            ...
            <div className="flex gap-3">
              <div className="flex-1 bg-gray-50 rounded-lg px-3 py-2">
                ...
                <p className="text-xs text-gray-600">{formData.latitude.toFixed(6)}</p>
              </div>
              <div className="flex-1 bg-gray-50 rounded-lg px-3 py-2">
                <span className="text-[10px] font-bold text-gray-400">Lng</span>
                <p className="text-xs text-gray-600">{formData.longitude.toFixed(6)}</p>
              </div>
            </div>

            <div className="flex gap-3">
              <button type="button" onClick={() => setStep(1)} className="flex-1 border border-gray-200 text-gray-600 py-3 rounded-xl font-bold text-sm active:scale-[0.98]">
                ← পূর্ববর্তী
              </button>
              <button type="button" onClick={() => setStep(3)} className="flex-1 bg-red-600 text-white py-3 rounded-xl font-bold text-sm active:scale-[0.98]">
                পরবর্তী →
              </button>
            </div>
          </div>
        ...
        {step === 3 && (
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-2">ছবি আপলোড</label>
              <div className="grid grid-cols-4 gap-2">
                {images.map((img, i) => (
                  <div key={i} className="relative aspect-square rounded-xl overflow-hidden border border-gray-200">
                    <img src={URL.createObjectURL(img)} alt="Preview" className="w-full h-full object-cover" />
                    <button type="button" onClick={() => removeImage(i)} className="absolute top-1 right-1 bg-black/60 text-white p-0.5 rounded-full">
                      <X size={12} />
                    </button>
                  </div>
                ))}
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-2">এক্সটার্নাল লিঙ্ক</label>
                ...
                {externalLinks.map((link, i) => (
                  <div key={i} className="flex gap-2">
                    <div className="flex-1 relative">
                      ...
                      <input
                        type="url"
                        placeholder="ভিডিও বা নিউজ লিঙ্ক"
                        value={link}
                    ...
                    {externalLinks.length > 1 && (
                      <button type="button" onClick={() => removeExternalLink(i)} className="text-gray-400 hover:text-red-600">
                        <X size={18} />
                      </button>
                    )}
                  </div>
                ))}
                <button type="button" onClick={addExternalLink} className="text-red-600 text-xs font-bold hover:underline">
                  + আরও লিঙ্ক
                </button>
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <button type="button" onClick={() => setStep(2)} className="flex-1 border border-gray-200 text-gray-600 py-3 rounded-xl font-bold text-sm active:scale-[0.98]">
                ← পূর্ববর্তী
              </button>
              <button
                type="submit"
